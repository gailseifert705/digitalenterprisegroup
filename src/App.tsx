...
const platformLogos: Array<{ title: string; image: string; href?: string; description?: string }> = [
...
function Header() {
  ...
  type HeaderLink = { label: string; href: string; group?: string };
  ...
  const headerItems: HeaderItem[] = [
    ...
    { label: "Automation", href: "/#automation" },
    {
      ...
      children: [
        ...
        { label: "CRM Automation & Workflows", href: "/crm-automation/" },
        { label: "Systems & CRM Integrations", href: "/web-app-integrations/" },
        { label: "Business Process Automation", href: "/business-process-automation/" },
        { label: "Other Core Services", href: "/#core-services" },
      ],
    },
    { label: "Projects", href: "/#projects" },
    {
              ...
              if ("children" in item) {
                ...
                return (
                  <div className={`nav-dropdown ${dropdownIsActive ? "is-active" : ""}`} key={item.label}>
                    ...
                    <div
                      className={`nav-dropdown-menu ${item.children.some((child) => child.group) ? "has-groups" : ""}`}
                      role="menu"
                      aria-label={`${item.label} submenu`}
                    >
                      {item.children.some((child) => child.group) ? (
                        <div className="nav-dropdown-columns">
                          {[...new Set(item.children.map((child) => child.group).filter(Boolean))].map((group) => (
                            <div className="nav-dropdown-column" key={group}>
                              <span className="nav-dropdown-group-label">{group}</span>
                ...
                return (
                  <div className="mobile-dropdown-group" key={item.label}>
                    ...
                    <div>
                      {item.children.some((child) => child.group) ? (
                        [...new Set(item.children.map((child) => child.group).filter(Boolean))].map((group) => (
                          <div className="mobile-dropdown-subgroup" key={group}>
                            <small>{group}</small>
                            {item.children.filter((child) => child.group === group).map((child) => (
                              <a
                                key={`${group}-${child.label}`}
...
function CrmSeoOverview() {
  const items = [
    {
      eyebrow: "CRM IMPLEMENTATION",
      title: "CRM setup and consulting built around the way your team sells.",
      text: "Digital Enterprise Group helps businesses plan, configure, clean up, migrate, and manage CRM systems without forcing teams into a generic template. We structure pipelines, lifecycle stages, ownership rules, fields, dashboards, and handoffs around the real sales process so the CRM is easier to use and easier to maintain.",
      href: "/crm-setup-management/",
      link: "Explore CRM setup & management",
    },
    {
      eyebrow: "PLATFORM EXPERIENCE",
      title: "HubSpot, Pipedrive, Keap, GoHighLevel, and monday.com support.",
      text: "Our CRM implementation work can include HubSpot setup, Pipedrive consulting, Keap configuration, GoHighLevel CRM setup, and monday.com CRM implementation. We can also help with data cleanup, migrations, pipeline configuration, user adoption, reporting, and ongoing CRM management as the business grows.",
      href: "/crm-setup-management/#platforms",
      link: "See CRM platform capabilities",
    },
    {
      eyebrow: "AUTOMATION & INTEGRATION",
      title: "Connect the CRM to the workflows and tools that run the business.",
      text: "Beyond CRM setup, DEG builds CRM automations, system workflows, API integrations, lead routing, follow-up sequences, notifications, approvals, and connected business processes. Our Las Vegas-based team helps businesses reduce repetitive work while keeping the systems practical for the people who use them every day.",
      href: "/crm-automation/",
      link: "Explore CRM automation",
    },
  ];

  return (
    <section className="crm-seo-overview" aria-labelledby="crm-seo-overview-title">
      <div className="wrap">
        <motion.div className="crm-seo-overview-head" {...fadeUp()}>
          <span className="eyebrow">CRM SYSTEMS &amp; AUTOMATION</span>
          <h2 id="crm-seo-overview-title">CRM implementation, automation, and integrations for growing businesses.</h2>
          <p>
            Digital Enterprise Group is a Las Vegas-based CRM and business automation team helping companies build cleaner systems, more reliable workflows, and better visibility across sales and operations.
          </p>
        </motion.div>

        <div className="crm-seo-overview-grid">
          {items.map((item, index) => (
            <motion.article key={item.eyebrow} className="crm-seo-overview-card" {...fadeUp(index * 0.08)}>
              <span>{item.eyebrow}</span>
              <h3>{item.title}</h3>
              <p>{item.text}</p>
              <a href={item.href}>{item.link} <ArrowUpRight size={15} /></a>
            </motion.article>
          ))}
        </div>
      </div>
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <select name="service" defaultValue={defaultService}>
                ...
                <option>CRM Setup & Management Services</option>
                <option>CRM Automation Services</option>
                <option>Systems & CRM Integration Services</option>
...
function ServiceSystemVisual({ page }: { page: ServicePage }) {
  ...
  return (
    <motion.aside
      className="ux-service-brief"
      initial={{ opacity: 0, y: 28 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, delay: 0.14, ease: [0.16, 1, 0.3, 1] }}
      aria-label={`${page.serviceLabel} engagement overview`}
    >
      <div className="ux-service-brief-head">
        <span>Working brief</span>
        <small>DEG / systems practice</small>
      </div>

      <div className="ux-service-brief-statement">
        <div className="ux-service-brief-icon"><Icon size={28} /></div>
        <div>
          <p>Service focus</p>
          <h2>{page.serviceLabel}</h2>
        </div>
      </div>

      <p className="ux-service-brief-copy">{page.shortDescription}</p>

      <div className="ux-service-brief-flow" aria-label="Expected engagement outcomes">
      ...
      <div className="ux-service-brief-foot">
        <span>Typical direction</span>
        <strong>{page.accentLabel}</strong>
      </div>
    </motion.aside>
  );
}
...
function ServiceProcessExperience({ page }: { page: ServicePage }) {
  ...
  return (
    <div className="ux-process-layout">
      <div className="ux-process-nav" role="tablist" aria-label={`${page.eyebrow} process steps`}>
      ...
      <motion.article
        ...
        className="ux-process-detail"
        initial={{ opacity: 0, y: 18 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.42, ease: [0.16, 1, 0.3, 1] }}
      >
        <div className="ux-process-detail-top">
          <span>Step {active.step}</span>
          <div><ActiveIcon size={25} /></div>
        </div>
        <h3>{active.title}</h3>
        <p>{active.text}</p>
        <div className="ux-process-detail-line" aria-hidden="true">
          <motion.i
            key={activeStep}
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
        ...
        <small>Clear scope · Practical configuration · Team handoff</small>
      </motion.article>
    </div>
  );
}
...
const serviceRailItems = [
  ...
  { id: "service-problems", label: "Challenges" },
  { id: "deliverables", label: "Solutions" },
  { id: "service-process", label: "Process" },
  { id: "service-faq", label: "FAQ" },
];

function ServicePageRail() {
  const [activeId, setActiveId] = useState("service-top");

  useEffect(() => {
    const sections = serviceRailItems
  ...
  return (
    <nav className="service-page-rail" aria-label="Service page section navigation">
      <span className="service-page-rail-line" aria-hidden="true" />
      {serviceRailItems.map((item, index) => (
        <button
          key={item.id}
          type="button"
...
function ServicePageView({ page }: { page: ServicePage }) {
  ...
  return (
    <div className="app-shell service-page-app ux-service-page" data-service={page.slug}>
      <ScrollProgress />
      <Header />
      <main>
        <section id="service-top" className="ux-service-hero">
          <div className="service-detail-shell">
            <nav className="service-breadcrumbs" aria-label="Breadcrumb">
              <a href="/">Home</a><ChevronRight size={14} /><a href="/#services">Services</a><ChevronRight size={14} /><span>{page.serviceLabel}</span>
            </nav>

            <div className="ux-service-hero-grid">
              <motion.div
                className="ux-service-hero-copy"
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.82, ease: [0.16, 1, 0.3, 1] }}
              >
                <div className="ux-service-kicker">
                ...
                <div className="service-detail-actions">
                  ...
                  <button className="service-detail-text-link" type="button" onClick={() => scrollToId("#deliverables")}>See the scope <ChevronRight size={17} /></button>
                </div>
                <div className="ux-service-proofline">
                  <span><MapPin size={15} /> Las Vegas-based</span>
                  <span><ShieldCheck size={15} /> Implementation-led</span>
                  <span><CheckCircle2 size={15} /> Built around real workflows</span>
                </div>
              </motion.div>
        ...
        <section className="ux-service-intro" aria-label={`${page.primaryKeyword} overview`}>
          <div className="service-detail-shell ux-service-intro-grid">
            <motion.div className="ux-service-intro-title" {...fadeUp()}>
              <span className="eyebrow">The engagement</span>
              <h2>Start with the work. Then shape the system around it.</h2>
            </motion.div>
            <motion.div className="ux-service-intro-body" {...fadeUp(0.08)}>
              <p>{page.shortDescription}</p>
              {page.secondaryKeywords && page.secondaryKeywords.length > 0 && (
                <div className="ux-service-coverage" aria-label="Service coverage">
                  {page.secondaryKeywords.slice(0, 6).map((keyword, index) => (
                    <div key={keyword}><span>{String(index + 1).padStart(2, "0")}</span><strong>{keyword}</strong></div>
                  ))}
                </div>
              )}
            </motion.div>
        ...
        <section id="service-problems" className="ux-service-section ux-service-problems">
          <div className="service-detail-shell">
            <motion.div className="ux-section-heading" {...fadeUp()}>
              <span className="eyebrow">Where the process breaks</span>
              <h2>{page.sectionHeadings?.problems || "Manual systems create hidden delays and inconsistent work."}</h2>
              <p>We look for the operational friction behind the tool—not just the symptoms inside the software.</p>
            </motion.div>

            <div className="ux-problem-list">
              {page.challenges.map((item, index) => (
                <motion.article key={item.title} {...fadeUp(index * 0.05)}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <h3>{item.title}</h3>
                  <p>{item.text}</p>
                </motion.article>
        ...
        <section id="service-outcomes" className="ux-service-section ux-service-outcomes">
          <div className="service-detail-shell ux-service-outcomes-grid">
            <motion.div className="ux-service-outcomes-copy" {...fadeUp()}>
              <span className="eyebrow dark-eyebrow">What better looks like</span>
              <h2>{page.sectionHeadings?.outcomes || "A more connected way to manage the work."}</h2>
              <p>Good automation should make the next step obvious, keep the right people informed, and make the system easier to trust.</p>
              <div className="ux-service-outcomes-mark"><Icon size={32} /><span>{page.accentLabel}</span></div>
            </motion.div>

            <div className="ux-outcome-list">
              {page.outcomes.map((item, index) => (
                <motion.article key={item.title} {...fadeUp(index * 0.06)}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
        ...
        <section id="deliverables" className="ux-service-section ux-service-deliverables">
          <div className="service-detail-shell">
            <motion.div className="ux-section-heading ux-section-heading-split" {...fadeUp()}>
              <div>
                <span className="eyebrow">What we can build</span>
                <h2>{page.sectionHeadings?.deliverables || `${page.eyebrow} deliverables.`}</h2>
              </div>
              <p>The scope is shaped around your current tools, team responsibilities, data, and the first business outcome that matters most.</p>
            </motion.div>

            <div className="ux-deliverable-list">
              {page.deliverables.map((item, index) => (
                <motion.article key={item.title} {...fadeUp(index * 0.05)}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <h3>{item.title}</h3>
                  <p>{item.text}</p>
                  <div>{item.tags.map((tag) => <i key={tag}>{tag}</i>)}</div>
                </motion.article>
              ))}
            </div>
          </div>
        ...
        {page.platformFocus && page.platformFocus.length > 0 && (
          <section id="crm-platforms" className="ux-service-section ux-platform-section">
            <div className="service-detail-shell ux-platform-layout">
              <motion.div className="ux-platform-intro" {...fadeUp()}>
                <span className="eyebrow">CRM platform services</span>
                <h2>One operating model, configured for the platform you use.</h2>
                <p>We do not force the same setup into every CRM. The pipeline, data model, permissions, reporting, and automation are adapted to the strengths of each platform.</p>
              </motion.div>

              <div className="ux-platform-list">
                {page.platformFocus.map((platform, index) => (
                  <motion.article key={platform.name} {...fadeUp(index * 0.05)}>
                    <span>{String(index + 1).padStart(2, "0")}</span>
                    <div className="ux-platform-name"><strong>{platform.name}</strong><small>{platform.title}</small></div>
                    <p>{platform.text}</p>
                    <div>{platform.keywords.map((keyword) => <i key={keyword}>{keyword}</i>)}</div>
                  </motion.article>
                ))}
              </div>
            </div>
        ...
        <section className="ux-service-section ux-use-cases">
          <div className="service-detail-shell ux-use-cases-grid">
            <motion.div className="ux-use-cases-copy" {...fadeUp()}>
              <span className="eyebrow">A practical first phase</span>
              <h2>{page.sectionHeadings?.useCases || "Start with the process creating the most friction."}</h2>
              <p>You do not need to rebuild everything at once. A focused first phase can solve an immediate problem and create a stronger foundation for the next system.</p>
              <a href="/#contact">Talk through the priority <ArrowUpRight size={17} /></a>
            </motion.div>

            <motion.ol className="ux-use-case-list" {...fadeUp(0.08)}>
        ...
        <section id="service-process" className="ux-service-section ux-service-process">
          <div className="service-detail-shell">
            <motion.div className="ux-section-heading" {...fadeUp()}>
              <span className="eyebrow">How the work moves</span>
              <h2>{page.sectionHeadings?.process || "From business problem to working system."}</h2>
              <p>Each phase produces a clear decision or working output before the next layer is added.</p>
            </motion.div>
            ...
            <div>
              {platformLogos.map((platform) => (
                <figure key={platform.title}>
                  {platform.href ? (
                    <a href={platform.href} aria-label={`Explore ${platform.title} CRM services`}>
                      <img
                        src={platform.image}
        ...
        <section id="service-faq" className="ux-service-section ux-service-faq">
          <div className="service-detail-shell ux-service-faq-grid">
            <motion.div className="ux-service-faq-copy" {...fadeUp()}>
              <span className="eyebrow dark-eyebrow">Before we begin</span>
              <h2>{page.sectionHeadings?.faq || `Questions about ${page.eyebrow.toLowerCase()}.`}</h2>
              <p>These are common planning questions. The final recommendation depends on your process, data, team, and current tools.</p>
            </motion.div>
            <div className="service-faq-list">
              {page.faqs.map((faq, index) => (
                <motion.details key={faq.question} {...fadeUp(index * 0.05)}>
        ...
        <section className="ux-service-section ux-related-services">
          <div className="service-detail-shell">
            <motion.div className="ux-section-heading ux-section-heading-split" {...fadeUp()}>
              <div><span className="eyebrow">Related services</span><h2>Build the next connected layer.</h2></div>
              <p>Once the core system is working, the next improvement is usually an integration, workflow, or adjacent business process.</p>
            </motion.div>
            <div className="ux-related-grid">
              {relatedPages.map((related, index) => {
                ...
                return (
                  <motion.a key={related.slug} href={related.path} {...fadeUp(index * 0.06)}>
                    <span>{String(index + 1).padStart(2, "0")}</span>
                    <RelatedIcon size={25} />
                    <h3>{related.serviceLabel}</h3>
